...
struct new_movie * new_releasesList; /*new movie releases | sorted by mid */

struct user * userList; /*users | unsorted */

struct movie ** catTable; /*category table - > movies, sorted by mid*/
 ...
 * watch history stack
 */
void unregister_user(int uid);			/*ok*/
 ...
 */
int add_new_movie(unsigned mid, movieCategory_t category, unsigned year); /*ok*/
 ...
 */
void distribute_new_movies(void); /*ok*/
 ...
 */
int watch_movie(int uid, unsigned mid); /*ok*/
 ...
 * Returns 0 on success, -1 on failure
 */
int filtered_movie_search(int uid, movieCategory_t category1,	/*ok*/
 ...
 * users list
 */
void print_users(void);  /*ok*/

/*ADDITIONS*/

/*
* Prints the added movie and the mids of the movies in the new_releasesList
*/

void print_new_movies(unsigned mid, movieCategory_t category, unsigned year); /*ok*/


/*
* Prints the uids of the users in userList
*/

void print_just_user(void); /*ok*/

/*
* Prints suggested movies
*/

void print_suggested(struct user *);

/*
* Prints watch history
*/

void print_watch_history(struct user *);

struct movie * Push(struct movie_info , struct movie *);

struct movie * Pop(struct user *);

int insert(struct user *, struct movie_info );
