...
void init_structures(void)
{
	...
	userList = (struct user *)malloc(sizeof(struct user)); /*creating sentinel node*/
    userList->uid=-1;               
	...
	catTable = (struct movie **)malloc(6*(sizeof(struct movie *))); /*allocating memory for category table !!!!!*/
	
	int i;

	for(i=0; i<6; i++) {catTable[i]==NULL;}
...
{
	struct user * next;  /*users demolished*/
	while (userList->uid != -1) {
		next = userList->next;
		unregister_user(userList->uid);
		userList=next;
	}
	...
	struct new_movie * tmp;	/*new releases exterminated*/

	while (new_releasesList != NULL) {
		tmp = new_releasesList;
		new_releasesList = new_releasesList ->next;
		free(tmp);
	}

	new_releasesList = NULL; 
	int i;
	
	for (i=0; i<6; i++) {	/*category table annihilated*/

		struct movie * tmp;
		while (catTable[i]!=NULL) {
			tmp = catTable[i];
			catTable[i]=catTable[i]->next;
			free(tmp);
		}
		catTable[i]=NULL;
	}
